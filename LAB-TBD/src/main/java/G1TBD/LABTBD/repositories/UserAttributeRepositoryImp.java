 ...
 * Logger.getLogger(UserAttributeRepositoryImp.class.getName());
 * 
 * @Autowired
 ...
 * 
 * } catch (Exception e) {
 * logger.severe("Error al obtener todos los atributos de usuario: " +
 * e.getMessage());
 * return List.of();
 * }
 * }
 ...
 * String sql =
 * "SELECT * FROM User_Attribute WHERE idUserAttribute = :idUserAttribute";
 * 
 * try (Connection conn = sql2o.open()) {
 * return conn.createQuery(sql)
 ...
 * 
 * } catch (Exception e) {
 * logger.severe("Error al obtener atributo de usuario por id: " +
 * e.getMessage());
 * return null;
 * }
 * }
 ...
 * String sql =
 * "UPDATE User_Attribute SET rut = :rut, idAttribute = :idAttribute WHERE idUserAttribute = :idUserAttribute"
 * ;
 * 
 * try (Connection conn = sql2o.open()) {
 * conn.createQuery(sql)
 ...
 * String sql =
 * "DELETE FROM User_Attribute WHERE idUserAttribute = :idUserAttribute";
 * 
 * try (Connection conn = sql2o.open()){
 * conn.createQuery(sql)
 * .addParameter("idUserAttribute", id)
 ...
 * 
 * } catch (Exception e) {
 * logger.severe("Error al obtener atributos de usuario por rut: " +
 * e.getMessage());
 * return List.of();
 * }
 * }
 * 
 * @Override
 * public List<UserAttributeEntity> getByattribute_id(long attribute_id) {
 * String sql = "SELECT * FROM User_Attribute WHERE idAttribute = :idAttribute";
 * 
 * try (Connection conn = sql2o.open()) {
 * return conn.createQuery(sql)
 * .addParameter("idAttribute", attribute_id)
 * .executeAndFetch(UserAttributeEntity.class);
 * 
 * } catch (Exception e) {
 * logger.severe("Error al obtener atributos de usuario por id de atributo: " +
 * e.getMessage());
 * return List.of();
 * }
 * }
